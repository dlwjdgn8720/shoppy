...
export const getReady = async (req, res, next) => {
  //console.log("req kakao", req.body);
  ...
  try {
    ...
    const data = {
      ...
      vat_amount: "0",
      tax_free_amount: "0",
      approval_url: `https://quickly-deserve-contort.ngrok-free.dev/kakao/approve?partner_order_id=${orderId}`, //카카오에서 redirection url
      fail_url: "http://192.168.7.85:3000/fail",
      cancel_url: "http://192.168.7.85:3000/cancel",
    };
    ...
    res.json({ tid: tid, qrUrl: next_redirect_mobile_url });
  } catch (error) {
    console.log("kakao pay ready error:::", error);
  }
};
...
export const getApprove = async (req, res, next) => {
  ...
  try {
    ...
    //결제완료 후 자동으로 http://192.168.7.85:3000/success 페이지로 자동 이동!!
    //http://192.168.7.85:3000/success 페이지 이동 코드 추가
    //res.redirect('http://192.168.7.85:3000/success'); -> App.js에 등록필요
  } catch (error) {
